...
def get_df_CNR_phase(Data_to_amp,loaded_nodes_loc,fixed_y=0.012, fixed_x=0.012, rect_size=5, margin=6):
    df_SNR = pd.DataFrame(columns=["SNR", "phase_difference"])  # Create an empty DataFrame

    for file in os.listdir(Data_to_amp):
        df_path = os.path.join(Data_to_amp, file)
        df_res = pd.read_csv(df_path)
        ...
        df_SNR = df_SNR.append({"SNR": snr_values, "phase_difference": phase_diff_values}, ignore_index=True)
    return df_SNR

def get_df_CNR_Amp(Data_to_amp,loaded_nodes_loc, fixed_y=0.012, fixed_x=0.012, rect_size=5, margin=6):
    df_SNR_Amp = pd.DataFrame(columns=["SNR", "phase_difference"])  # Create an empty DataFrame
    ...
    for file in os.listdir(Data_to_amp):
        ...
        df_SNR_Amp = df_SNR_Amp.append({"SNR": snr_values, "phase_difference": phase_diff_values}, ignore_index=True)
    return df_SNR_Amp