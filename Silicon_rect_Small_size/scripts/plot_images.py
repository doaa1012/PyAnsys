...
def plot_images(node_temp_from_post_top, nodes_loc, label="Temperature", plot_image=True):
    ...
    #image_filename = f'temperature_plot_{i}.png'
    grid_temperature = griddata((x_values, y_values), temperature_values, (grid_x, grid_y), method='linear')
    #colormap_data = plt.cm.viridis((temperature_values - temperature_values.min()) / (temperature_values.max() - temperature_values.min()))
    if plot_image == True:
        plt.figure(figsize=(8, 6))
        plt.imshow(grid_temperature, extent=(x_min, x_max, y_min, y_max), cmap="plasma", origin="upper", aspect="auto")
        ...
        plt.axis("off")
        plt.savefig("image_filename", dpi=300, bbox_inches='tight')  # Save the image
        plt.show()
